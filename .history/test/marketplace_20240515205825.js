...
describe("Deploying Marketplace contract", async () => {
  ...
  let marketplaceAddress, deployer, user1, user2, user3, user4, user5, user6, tranasaction, result, URI;
  let marketplace;
  let nft;
  ...
  beforeEach(async () => {
    [marketplaceAddress, , ,deployer, user1, user2, user3, user4, user5, user6] = await ethers.getSigners();
  ...
  it('lists that minted nft onto the marketplace contract.', async () => {
    ...
    await nft.connect(user1).approve(deployer, 1)
